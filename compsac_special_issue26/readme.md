...
- **What it does:** The script scans `dataset/1.code_smell_tf_file` for `.tf` files. Files prefixed with `f_` are no smell and files prefixed with `t_` are smell present samples. The smell label is inferred from the parent subfolder name (for `t_` files).
- **Run it:**
